...
export const TripDetailPage: React.FC = () => {
    ...
    // Smooth back to top on page load
    useEffect(() => {
    ...
    // Fallback default details if the route is not found in mockRoutes
    const defaultRoute = {
        id: 'default',
        from: 'Hà Nội',
        to: 'TP. Hồ Chí Minh',
    ...
    const currentRoute = route 
        ? {
            ...
            rating: '4.8 (1.2k đánh giá)',
            onTimeRate: '98%',
            licensePlate: route.id === '1' ? '29B-999.99' : '29B-123.45',
            driverName: 'Nguyễn Văn A',
            driverAvatar: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDCZxf62S9I9m2-P4u-TNZuGrLkl8WUlgQizhvlL1guwYbL-ZJ1BqFajUfg8BJqJiG5appsQ3DMgzZdrC1sA7ldjzmioHyvW9XUZfYKgMzeVE_1CeVKI31dBA_X-mgJIVEhhsWhtWE3w-hEBZ3TS744HASHOugiXk3y2uUO5juOOaMpmvNFSnciC3Ak2vqpTpWckSLvkeQPiGgA4La5pzCKLbXpQ1TQV26Nj5Zgp9_HfnP2au8DwPNmaJBUoe8tXlhn9vsIPunX4w8'
          }
        : defaultRoute;
    ...
    const itinerary = [
        {
            name: currentRoute.from === 'Hà Nội' ? 'Bến xe Mỹ Đình' : `Bến xe ${currentRoute.from}`,
            time: '06:30',
            location: currentRoute.from,
            details: 'Khởi hành chuyến đi',
            type: 'major'
        },
        {
            name: currentRoute.from === 'Hà Nội' ? 'Bến xe Yên Nghĩa (Hà Đông)' : `Điểm đón Quốc Lộ`,
            time: '07:15',
        ...
        {
            name: currentRoute.to === 'TP. Hồ Chí Minh' ? 'Bến xe Miền Đông mới' : `Bến xe ${currentRoute.to}`,
            time: '22:30',
            location: currentRoute.to,
    ...
    const handleSelectSeat = () => {
        navigate(`/tuyen-duong/${currentRoute.id}/chon-ghe`, {
            state: { passengers }
        });
    };
        ...
        <div className="min-h-screen bg-background text-on-background font-body select-none">
            ...
            <main className="flex-grow container mx-auto max-w-container-max px-gutter py-lg pt-28">
                ...
                <div className="mb-md flex items-center space-x-2 text-on-surface-variant font-label-sm text-sm">
                    ...
                    <span className="material-symbols-outlined text-[16px]">chevron_right</span>
                    <span className="text-on-surface font-semibold">{currentRoute.from} - {currentRoute.to}</span>
                </div>
                        ...
                        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_8px_20px_rgba(92,64,51,0.03)] p-md text-left">
                            <div className="flex justify-between items-start border-b border-outline-variant pb-sm mb-sm">
                                <div className="flex items-center space-x-md">
                                    ...
                                    <div>
                                        <h1 className="typo-headline-md text-headline-md text-on-surface">{currentRoute.operator}</h1>
                                        <div className="flex items-center space-x-2 mt-xs">
                                            <div className="flex items-center text-secondary-container">
                                                <Star size={16} weight="fill" className="text-amber-500" />
                                                <span className="typo-label-sm text-sm text-on-surface ml-1">{currentRoute.rating}</span>
                                            </div>
                                            ...
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-surface-container-low text-primary border border-outline-variant text-xs">
                                                <CheckCircle size={14} className="mr-1" /> Đúng giờ {currentRoute.onTimeRate}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center justify-between py-sm">
                                ...
                                <div className="text-center w-1/4">
                                    ...
                                    <div className="typo-body-md text-sm text-on-surface-variant mt-1">{currentRoute.from === 'Hà Nội' ? 'Bến xe Mỹ Đình' : `Bến xe ${currentRoute.from}`}</div>
                                </div>

                                {/* Journey Path */}
                                <div className="w-2/4 flex flex-col items-center justify-center px-md">
                                    <div className="typo-label-sm text-sm text-on-surface-variant mb-2">~{currentRoute.duration}</div>
                                ...
                                <div className="text-center w-1/4">
                                    ...
                                    <div className="typo-body-md text-sm text-on-surface-variant mt-1">{currentRoute.to === 'TP. Hồ Chí Minh' ? 'Bến xe Miền Đông' : `Bến xe ${currentRoute.to}`}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-sm">
                            ...
                            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-sm flex items-center space-x-md text-left">
                                ...
                                <div>
                                    ...
                                    <div className="typo-body-lg text-base font-semibold text-on-surface">{currentRoute.vehicleType}</div>
                                </div>
                            </div>
                            ...
                            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-sm flex items-center space-x-md text-left">
                                ...
                                <div>
                                    ...
                                    <div className="flex space-x-2 mt-1 text-on-surface">
                                        <Usb size={18} title="Sạc USB" className="text-on-surface-variant" />
                                        <Bed size={18} title="Chăn" className="text-on-surface-variant" />
                                        <Drop size={18} title="Nước uống" className="text-on-surface-variant" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-sm flex items-center space-x-md text-left">
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 inline-block border-2 border-on-surface rounded bg-surface-container-lowest px-2 py-0.5">
                                        <span className="typo-body-lg text-sm font-bold text-on-surface tracking-widest">{currentRoute.licensePlate}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-sm flex items-center space-x-md text-left">
                                <div className="w-12 h-12 rounded-full bg-surface-container-high overflow-hidden shrink-0">
                                    <img alt="Driver" className="w-full h-full object-cover" src={currentRoute.driverAvatar} />
                                </div>
                                <div>
                                    <div className="typo-label-sm text-xs text-on-surface-variant">Tài xế</div>
                                    <div className="typo-body-lg text-base font-semibold text-on-surface">{currentRoute.driverName}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="sticky top-28 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_8px_20px_rgba(92,64,51,0.03)] p-md flex flex-col text-left">
                            ...
                            <div className="space-y-sm mb-md flex-grow">
                                <div className="flex justify-between items-center border-b border-outline-variant pb-xs">
                                    ...
                                    <span className="typo-body-md text-sm text-on-surface font-semibold">{currentRoute.from} - {currentRoute.to}</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center border-b border-outline-variant pb-xs mt-2">
                                    ...
                                    <span className="typo-body-md text-sm text-on-surface">{currentRoute.vehicleType}</span>
                                </div>
                                <div className="flex justify-between items-center border-b border-outline-variant pb-xs mt-2">
                                ...
                                <div className="flex justify-between items-center pt-xs mt-6">
                                    ...
                                    <span className="typo-headline-md text-xl font-bold text-primary">{(currentRoute.price * passengers).toLocaleString()}đ</span>
                                </div>
                            </div>
