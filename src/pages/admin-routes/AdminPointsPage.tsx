...
export const AdminPointsPage: React.FC = () => {
  ...
  const openModal = (point?: PointResponse) => {
    if (point) {
      ...
      setFormData({ name: '', address: '', cityId: cities[0]?.id || '', description: '', latitude: 0, longitude: 0 });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <AdminLayout>
      ...
      {/* Tabs */}
      <div className="flex border-b border-slate-300 gap-8 mb-6">
        <button
          className={`py-3 px-1 font-medium transition-colors border-b-2 ${
            activeTab === 'PICKUP' ? 'border-[#F4600C] text-[#F4600C]' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        ...
        <button
          className={`py-3 px-1 font-medium transition-colors border-b-2 ${
            activeTab === 'DROPOFF' ? 'border-[#F4600C] text-[#F4600C]' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">Chưa có dữ liệu.</td></tr>
            ) : points.map((point) => (
